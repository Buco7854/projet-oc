...
\begin{abstract}
Dans ce rapport, nous présentons notre travail sur la résolution du problème de tournées de véhicules avec contraintes de capacité (CVRP). Nous avons choisi d'implémenter une approche par colonies de fourmis (ACO), puis de l'améliorer en utilisant du Q-Learning pour ajuster automatiquement les paramètres de l'algorithme pendant la recherche. L'idée principale est que, plutôt que de fixer les paramètres de l'ACO une fois pour toutes, on laisse un agent d'apprentissage par renforcement apprendre quelle stratégie adopter selon l'état de la recherche. Nous comparons ensuite les deux approches sur plusieurs instances de tailles différentes, et nous discutons de ce que le machine learning apporte réellement dans ce contexte.
\end{abstract}
...
Les problèmes d'optimisation combinatoire sont partout dans le monde réel : logistique, planification, allocation de ressources, etc. Parmi eux, le problème de tournées de véhicules (VRP) est un classique, et sa variante avec capacités (CVRP) est celle que l'on rencontre le plus souvent en pratique.
...
Le CVRP est NP-difficile. Concrètement, pour $n$ clients, le nombre de solutions possibles explose de façon factorielle. Même avec 50 clients, il est impossible d'énumérer toutes les solutions. La métaheuristiques va nous permettre d'explorer intelligemment ce très grand espace.

Le CVRP présente aussi un intérêt réel. Il correspond au problème que rencontrent les entreprises de livraison, comment organiser les tournées pour minimiser les coûts tout en respectant la capacité des camions ?

\section{Métaheuristique : Colonies de fourmis (ACO)}

\subsection{Principe général}

L'optimisation par colonies de fourmis (ACO), introduite par Dorigo et Di Caro \cite{dorigo1999aco}, s'inspire du comportement des fourmis dans la nature. Quand des fourmis cherchent de la nourriture, elles déposent des phéromones sur leur chemin. Les chemins les plus courts accumulent plus de phéromones (parce que les fourmis y passent plus souvent), ce qui attire davantage de fourmis : c'est un mécanisme d'auto-renforcement.
...
Nous avons choisi $\beta > \alpha$ pour que l'heuristique de distance ait plus d'influence que les phéromones au début, lorsque celles-ci ne contiennent pas encore beaucoup d'information. Le taux d'évaporation $\rho = 0.1$ signifie que 10\% des phéromones disparaissent à chaque itération : une valeur trop faible (ex. 0.01) figerait la recherche sur les premiers chemins trouvés, tandis qu'une valeur trop forte (ex. 0.5) effacerait l'information trop vite pour que les bons chemins se renforcent. La valeur 0.1 offre un équilibre que nous avons vérifié expérimentalement lors de nos tests préliminaires.
...
L'idée de combiner du Q-Learning avec une métaheuristique est quelque chose que nous avions dès le départ du projet. En cherchant dans la littérature, nous sommes tombés sur le survey de Yang et al.\ \cite{yang2024qlma} qui recense une vingtaine d'années de travaux sur le sujet. C'est en lisant ce papier et en faisant différentes recherches sur le web que nous avons structuré notre approche. Ce survey recense l'adaptation de paramètres par Q-Learning (voir section~4.1.1 de \cite{yang2024qlma}), la sélection d'opérateurs guidée par la Q-table (section~4.1.5), et l'utilisation de politiques $\varepsilon$-greedy pour gérer l'équilibre exploration/exploitation (section~4.1.4). Notre approche reprend ces principes et les adapte au contexte de l'ACO pour le CVRP.

Nous avons choisi le Q-Learning \cite{watkins1992qlearning} pour plusieurs raisons. D'abord, c'est un algorithme relativement simple à comprendre et à implémenter. Ensuite, notre problème d'ajustement de paramètres se formule naturellement comme un problème de décision séquentielle : à chaque itération, on observe l'état de la recherche et on décide quelle stratégie adopter.

Par rapport à une simple grille de recherche de paramètres (grid search), le Q-Learning a l'avantage d'être adaptatif : il peut changer de stratégie au cours de la résolution, par exemple explorer au début puis intensifier vers la fin.
...
Les valeurs numériques de chaque profil découlent directement de la logique de la formule de probabilité $p_{ij} \propto \tau_{ij}^\alpha \cdot \eta_{ij}^\beta$ :

\begin{itemize}
    \item \textbf{Exploitation} ($\alpha=2.0$, $\beta=2.5$, $\rho=0.05$) : un $\alpha$ élevé amplifie l'écart entre les arcs très phéromonés et les autres, ce qui force les fourmis à suivre les chemins déjà établis. Le $\rho$ faible conserve ces phéromones plus longtemps.
    \item \textbf{Exploration} ($\alpha=0.5$, $\beta=4.0$, $\rho=0.20$) : un $\alpha$ faible réduit l'influence des phéromones, donc les fourmis ne sont plus attirées par les chemins connus. Le $\beta$ élevé les guide quand même vers les clients proches (pour éviter des solutions absurdes), et le $\rho$ élevé efface rapidement les anciennes traces.
    \item \textbf{Balanced} ($\alpha=1.0$, $\beta=3.0$, $\rho=0.10$) : ce sont les paramètres par défaut de notre ACO, un équilibre entre phéromones et heuristique.
    \item \textbf{Intensify} ($\alpha=2.5$, $\beta=2.0$, $\rho=0.02$) : version plus agressive que \texttt{exploitation}. Le $\rho$ très faible (2\% d'évaporation) conserve les phéromones au maximum pour renforcer les meilleurs chemins trouvés.
    \item \textbf{Diversify} ($\alpha=0.3$, $\beta=5.0$, $\rho=0.30$) : version plus agressive que \texttt{exploration}. Le $\rho$ à 0.30 efface 30\% des phéromones par itération, ce qui ``remet à zéro'' la mémoire collective et force la découverte de nouvelles zones.
    \item \textbf{Local\_search} ($\alpha=1.0$, $\beta=3.0$, $\rho=0.10$) : paramètres équilibrés, mais déclenche en plus le 2-opt (voir section suivante).
\end{itemize}

Les plages de valeurs ($\alpha \in [0.3, 2.5]$, $\beta \in [2.0, 5.0]$, $\rho \in [0.02, 0.30]$) ont été déterminées par des tests préliminaires : en dehors de ces bornes, l'algorithme produit soit des solutions de mauvaise qualité ($\alpha > 3$ fige la recherche), soit une convergence trop lente ($\rho > 0.5$ empêche tout apprentissage collectif).
...
La mise à jour suit la règle standard du Q-Learning \cite{watkins1992qlearning} :
...
Le 2-opt est un opérateur d'amélioration locale assez courant. L'idée dans notre cas est simple : on sélectionne deux arcs dans une route et on inverse le segment entre eux pour vérifier si cela réduit la distance totale. Ce processus est répété tant que des améliorations sont trouvées.

Nous l'appliquons uniquement lorsque le Q-Learning choisit la stratégie \texttt{local\_search}. Ce choix est délibéré : le 2-opt est coûteux en temps de calcul, et nous ne souhaitons donc pas l'appliquer à chaque itération. C'est au Q-Learning de décider quand son application est pertinente (voir section~4.1.5 de \cite{yang2024qlma} pour d'autres exemples de sélection d'opérateurs par Q-Learning).
...
Le tableau~\ref{tab:results} présente nos résultats. Désolé si il n'est pas bien centré il y a beaucoup de colones.
...
\begin{itemize}
    \item Sur les petites instances (10--15 clients), les deux approches donnent des résultats quasiment identiques. Cela est cohérent : l'espace de recherche est restreint, et l'ACO classique trouve déjà de très bonnes solutions. Le Q-Learning n'a pas de marge d'amélioration significative.

    \item Sur les instances moyennes (20--30 clients), les résultats sont mitigés. Sur 20 clients, l'hybride est légèrement moins performant en moyenne ($-0.55\%$), mais sur 25 et 30 clients, il fait mieux ($+1.05\%$ et $+0.99\%$). Le résultat négatif sur 20 clients s'explique probablement par le fait que l'exploration du Q-Learning introduit de la variance sans apporter suffisamment de bénéfice sur une instance de cette taille.

    \item Sur les grandes instances (40--50 clients), l'hybride montre clairement son avantage. Le meilleur résultat est obtenu sur 40 clients avec $+3.49\%$ d'amélioration. Cela est cohérent avec l'intuition : plus l'espace de recherche est vaste, plus l'ajustement dynamique des paramètres peut aider à explorer différentes zones.
\end{itemize}
...
La figure~\ref{fig:convergence} présente la convergence réelle des deux approches sur l'instance \texttt{large\_40} (40 clients, seed 100). On constate que l'ACO classique converge rapidement vers un plateau autour de l'itération 30 et stagne ensuite. L'hybride, grâce aux changements de stratégie pilotés par le Q-Learning, parvient à rompre cette stagnation à plusieurs reprises (notamment autour des itérations 55 et 75) et atteint un coût final sensiblement inférieur.
...
\addplot[blue!70!black, thick, smooth] coordinates {
    (0,1113.32) (5,1061.04) (10,957.28) (15,957.28) (20,949.32) (25,949.32) (30,867.06) (35,867.06) (40,867.06) (45,867.06) (50,867.06) (55,867.06) (60,867.06) (65,867.06) (70,867.06) (75,867.06) (80,867.06) (85,867.06) (90,867.06) (95,867.06) (100,867.06) (105,867.06) (110,867.06) (115,867.06) (120,867.06) (125,867.06) (130,867.06) (135,867.06) (140,867.06) (145,867.06) (149,867.06)
};
\addlegendentry{ACO classique (867.06)}
\addplot[green!60!black, thick, smooth, dashed] coordinates {
    (0,1113.32) (5,974.95) (10,974.95) (15,910.49) (20,910.49) (25,902.44) (30,886.70) (35,886.70) (40,886.70) (45,886.70) (50,886.70) (55,868.31) (60,868.31) (65,868.31) (70,868.31) (75,822.73) (80,822.73) (85,822.73) (90,822.73) (95,822.73) (100,822.73) (105,822.73) (110,822.73) (115,822.73) (120,816.95) (125,816.95) (130,816.95) (135,816.95) (140,816.95) (145,816.95) (149,816.95)
};
\addlegendentry{ACO + Q-Learning (816.95)}
\end{axis}
\end{tikzpicture}
\caption{Convergence réelle sur l'instance \texttt{large\_40} (seed 100)}
...
Notre approche a aussi des limites :
...
\begin{itemize}
    \item \textbf{Transfert d'apprentissage} : pré-entraîner la table Q sur un ensemble d'instances pour que l'agent dispose d'une bonne politique de départ lorsqu'il aborde une nouvelle instance. Cela pourrait réduire la phase d'exploration initiale.
    ...
    \item \textbf{Deep Q-Learning} : remplacer la table Q par un réseau de neurones pour gérer un espace d'états plus fin (features continues au lieu de discrétiser). Cela permettrait d'être plus précis dans la caractérisation de l'état de la recherche.
...
Ce projet nous a permis d'observer concrètement comment le machine learning peut interagir avec une métaheuristique. Nous pensons qu'avec des techniques plus avancées (deep RL, transfert d'apprentissage) et une validation sur des benchmarks standards, il serait possible d'aller plus loin.

\begin{thebibliography}{99}

\bibitem{yang2024qlma}
Y.~Yang, Y.~Gao, Z.~Ding, J.~Wu, S.~Zhang, F.~Han, X.~Qiu, S.~Gao et Y.-G.~Wang,
``Advancements in Q-learning meta-heuristic optimization algorithms: A survey,''
\textit{WIREs Data Mining and Knowledge Discovery}, vol.~14, no.~6, e1548, 2024.
\newline DOI: \url{https://doi.org/10.1002/widm.1548}

\bibitem{dorigo1999aco}
M.~Dorigo et G.~Di~Caro,
``Ant colony optimization: A new meta-heuristic,''
in \textit{Proceedings of the 1999 Congress on Evolutionary Computation (CEC'99)}, vol.~2, pp.~1470--1477, IEEE, 1999.
